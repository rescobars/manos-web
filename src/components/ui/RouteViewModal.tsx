...
import { XCircle, MapPin, Clock, Package, Route, Navigation, CheckCircle, AlertCircle } from 'lucide-react';
...
function MapContent({ route }: { route: RouteData }) {
  ...
  useEffect(() => {
    ...
    if (route.route_points && route.route_points.length > 0) {
      const routeCoordinates = route.route_points.map(point => 
    ...
    originMarker.bindPopup(`
      <div class="text-center min-w-[200px]">
        ...
        <div class="text-xs text-gray-500">${route.origin_lat.toFixed(6)}, ${route.origin_lon.toFixed(6)}</div>
      </div>
    `);
    ...
    destinationMarker.bindPopup(`
      <div class="text-center min-w-[200px]">
        ...
        <div class="text-xs text-gray-500">${route.destination_lat.toFixed(6)}, ${route.destination_lon.toFixed(6)}</div>
      </div>
    `);
    ...
    route.waypoints.forEach((waypoint, index) => {
      ...
      waypointMarker.bindPopup(`
        <div class="text-center min-w-[200px]">
          ...
          <div class="text-xs text-gray-500">${waypoint.lat.toFixed(6)}, ${waypoint.lon.toFixed(6)}</div>
        </div>
      `);
...
export function RouteViewModal({ routeUuid, onClose }: RouteViewModalProps) {
  ...
  if (isLoading) {
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col theme-bg-3" style={{ backgroundColor: colors.background3 }}>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="rounded-xl shadow-2xl max-w-7xl w-full max-h-[90vh] flex flex-col theme-bg-3" style={{ backgroundColor: colors.background3 }}>
        {/* Header */}
        <div className="p-4 sm:p-6 border-b theme-border" style={{ borderColor: colors.border }}>
          <div className="flex items-center justify-between">
            <div className="min-w-0 flex-1">
              <h3 className="text-lg sm:text-xl font-semibold theme-text-primary">{route.route_name}</h3>
              <p className="text-sm theme-text-secondary mt-1">{route.description}</p>
            </div>
            <button
              onClick={onClose}
                    ...
                    >
                      <div className="flex items-start justify-between">
                        ...
                        <div className="text-right">
                          <p className="text-lg font-semibold theme-success">${order.total_amount.toFixed(2)}</p>
